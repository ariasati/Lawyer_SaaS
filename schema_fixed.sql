/** 
* USERS (Base table from template - already exists)
* Note: This table contains user data. Users should only be able to view and update their own data.
*/
-- Users table should already exist from the base template
-- We'll add our custom columns after creating the firms table

/**
* This trigger automatically creates a user entry when a new user signs up via Supabase Auth.
*/ 
-- This should already exist from the base template

/**
* CUSTOMERS (Base table from template - already exists)
* Note: this is a private table that contains a mapping of user IDs to Stripe customer IDs.
*/
-- This should already exist

/** 
* PRODUCTS (Base table from template - already exists)
* Note: products are created and managed in Stripe and synced to our DB via Stripe webhooks.
*/
-- This should already exist

/**
* PRICES (Base table from template - already exists)
* Note: prices are created and managed in Stripe and synced to our DB via Stripe webhooks.
*/
-- This should already exist

/**
* SUBSCRIPTIONS (Base table from template - already exists)
...
/**
 * NOW add firm relationship to existing users table
 */
ALTER TABLE users ADD COLUMN IF NOT EXISTS firm_id uuid;
...
ALTER TABLE users ADD CONSTRAINT IF NOT EXISTS users_firm_id_fkey 
...
CREATE POLICY IF NOT EXISTS "Users can view own firm data" ON firms 
  FOR SELECT USING (id IN (SELECT firm_id FROM users WHERE id = auth.uid()));
CREATE POLICY IF NOT EXISTS "Users can update own firm data" ON firms 
  FOR UPDATE USING (id IN (SELECT firm_id FROM users WHERE id = auth.uid()));

-- Matters: Users can only access matters from their firm
CREATE POLICY IF NOT EXISTS "Users can view own firm matters" ON matters 
  FOR SELECT USING (firm_id IN (SELECT firm_id FROM users WHERE id = auth.uid()));
CREATE POLICY IF NOT EXISTS "Users can insert matters for own firm" ON matters 
  FOR INSERT WITH CHECK (firm_id IN (SELECT firm_id FROM users WHERE id = auth.uid()));
CREATE POLICY IF NOT EXISTS "Users can update own firm matters" ON matters 
...
CREATE POLICY IF NOT EXISTS "Users can insert own time entries" ON time_entries 
  FOR INSERT WITH CHECK (user_id = auth.uid());
CREATE POLICY IF NOT EXISTS "Users can update own time entries" ON time_entries 
  FOR UPDATE USING (user_id = auth.uid());

-- Invoices: Users can only access invoices from their firm
CREATE POLICY IF NOT EXISTS "Users can view own firm invoices" ON invoices 
  FOR SELECT USING (firm_id IN (SELECT firm_id FROM users WHERE id = auth.uid()));
CREATE POLICY IF NOT EXISTS "Users can insert invoices for own firm" ON invoices 
  FOR INSERT WITH CHECK (firm_id IN (SELECT firm_id FROM users WHERE id = auth.uid()));
CREATE POLICY IF NOT EXISTS "Users can update own firm invoices" ON invoices 
  FOR UPDATE USING (firm_id IN (SELECT firm_id FROM users WHERE id = auth.uid()));

-- Invoice Line Items: Users can only access line items from their firm's invoices
CREATE POLICY IF NOT EXISTS "Users can view own firm invoice line items" ON invoice_line_items 
  FOR SELECT USING (
    invoice_id IN (
      SELECT id FROM invoices WHERE firm_id IN (SELECT firm_id FROM users WHERE id = auth.uid())
    )
  );
CREATE POLICY IF NOT EXISTS "Users can insert line items for own firm invoices" ON invoice_line_items 
 ...
 * Allow realtime listening on lawyer billing tables
 */
